...
(define tiles-with-configurations
  (for/fold ((acc (hash))) ((tile data) (i (in-naturals)))
    (for/fold ((acc acc)) ((tile (configurations tile)) (ii (in-naturals)))
      (hash-set acc (cons i ii) tile))))

(define (connect? id1 id2 d)
  (equal? (border-indexes->string (hash-ref tiles-with-configurations id1) (hash-ref borders-indexes d))
          (border-indexes->string (hash-ref tiles-with-configurations id2) (hash-ref borders-indexes (* -1 d)))))

(define (find-adjacent id d)
  (for/first ((id2 (hash-keys tiles-with-configurations))
              #:when (and (connect? id id2 d) (not (= (car id) (car id2)))))
    id2))

(define (make-connections)
  (let loop ((acc (hash 0 (car (hash-keys tiles-with-configurations)))))
                 ...
                 (for/fold ((acc acc)) ((zz (list (- z 1) (+ z 1) (- z 0+i) (+ z 0+i))))
                   (if (hash-has-key? acc zz)
                       ...
                       (match (find-adjacent (hash-ref acc z) (- zz z))
                         (#f acc)
                         (e (hash-set acc zz e))))))))
      (if (= (hash-count acc) (length data))
          (for/hash ((z (hash-keys acc))) (values z (hash-ref tiles-with-configurations (hash-ref acc z))))
          (loop acc)))))
